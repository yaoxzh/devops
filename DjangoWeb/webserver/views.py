...
CurrentConfig.GLOBAL_ENV = Environment(loader=FileSystemLoader("./webserver/templates"))
...
def index(req):
    ...
    c = (
        Gauge()
            .add(series_name="cpu使用率", data_pair=[["", 55.5]])
            .set_global_opts(
    ...
    l = (
        Line()
            ...
            .set_global_opts(
            title_opts=opts.TitleOpts(title="cpu 使用率"),
    ...
    soup = BeautifulSoup(l.render_embed(), 'html.parser')

    # 获取div
    div_li_line = soup.find_all('div')
    div_elm_line = str(div_li_line[0])
    div_style_line = div_li_line[0].get('style')
    div_elm_line = div_elm_line.replace(div_style_line, "width:500px; height:300px;")

    # 获取script
    div_script_list_line = soup.find_all('script')
    div_script_elm_list_line = []
    for div_script_elm_line in div_script_list_line:
        div_script_elm_line = str(div_script_elm_line)
        div_script_elm_list_line.append(div_script_elm_line)
    ...
    # 获取div
    div_li_stack = soup.find_all('div')
    div_elm_stack = str(div_li_stack[0])
    div_style_stack = div_li_stack[0].get('style')
    div_elm_stack = div_elm_stack.replace(div_style_stack, "width:500px; height:300px;")

    # 获取script
    div_script_list_stack = soup.find_all('script')
    div_script_elm_list_stack = []
    for div_script_elm_stack in div_script_list_stack:
        div_script_elm_stack = str(div_script_elm_stack)
        div_script_elm_list_stack.append(div_script_elm_stack)

    # 组织内容
    content = {
        'div_elm': div_elm,
        'div_script_elm_list': div_script_elm_list,
        'div_elm_line': div_elm_line,
        'div_script_elm_list_line': div_script_elm_list_line,
        'OsVersion': OsVersion,
        'div_elm_stack': div_elm_stack,
        'div_script_elm_list_stack': div_script_elm_list_stack,
    }
...
def geThostIp(request):
    ...
    return render(request, 'menu.html', {'hostip': '192.168.16.131'})
...
def userAlter(req, id):
    ...
    if req.method == "POST":
        ...
        try:
            ...
            return render(req, 'useralter.html',
                          {'alter_FormInput': form, 'UpdateUser': UpdateUser, 'alter_is_active': old_is_active})
        except:
...
def getMonitor(request):
    ...
    '''获取提交的主机ID'''
    # 不是所有的牛奶都叫特仑苏，也不是所有的特仑苏都是好的牛奶
    # 今天我这么闲，可以说是闲的一批，明天，我必将慌得一批，如果有一天我累了，倒了，退下了，不能冻了，我必然要怎么怎么样
    # 我跟玉米已经在一起这么长的时间了，说实在话，两个人真的情同手足，寸步难离。如果真的要分开，只怕我会很伤心很伤心，真的很伤心
    try:
        if request.method == "POST":
